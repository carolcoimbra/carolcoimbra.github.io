...
reload_donut(0,0, "#squareThree");
reload_donut(0,0, "#squareThree2");

function reload_donut(idYear, reg, div){
	...
	if(div == "#squareThree")
	{
		...
		div_x.id = "squareThree";

		document.body.appendChild(div_x);
		
		d3.tsv(database,
		function(error, data, div) {
	   ...
	   var  arcs = vis.selectAll("g.slice")     //this selects all <g> elements with class slice (there aren't any yet)
		...
		arcs.append("svg:path")
			.attr("fill", function(d) { return color[d.data.ID-1];}) //set the color for each slice to be chosen from the color function defined above
		...
		arcs
			.on("mouseover",  function(d,i) {
				...
				.attr("style", "left:"+(d3.event.pageX-350)+"px;top:"+(d3.event.pageY-150)+"px")
	...
	else if( div == "#squareThree2")
	{
		...
		var tooltipDonut = d3.select(div_x).append("div").attr("class", "tooltipDonut hidden");
	   ...
	   var  arcs = vis.selectAll("g.slice")     //this selects all <g> elements with class slice (there aren't any yet)
		...
		div_y.id = "squareFour2";

		document.body.appendChild(div_y);


		var legend = d3.select("#squareFour2")
			.append("svg")
			.attr("width", 250)
			.attr("height",300);
		...
		arcs
			.on("mouseover",  function(d,i) {
				tooltipDonut.classed("hidden", false)
				.attr("style", "left:"+(d3.event.pageX-1000)+"px;top:"+(d3.event.pageY-150)+"px")
				.html("<strong>Year: "+Year[idYear]+"<br/>"+"</strong>"+"<strong>Subfunction: "+d.data.Subfunction+"<br/>"+"</strong>"+"<strong>Total: R$"+d.data.Total+"<br/>"+"</strong>")
			})
			.on("mouseout",  function(d,i) {
				tooltipDonut.classed("hidden", true)
			});	
